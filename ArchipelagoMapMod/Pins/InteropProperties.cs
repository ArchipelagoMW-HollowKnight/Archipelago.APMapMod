...
{
    // ArchipelagoMapMod currently has built-in metadata for the following connection mods:
    // - RandomizableLevers
    // - RandoPlus
    // - BenchRando
    ...
    /// <summary>
    ///     The name of the connection mod that this placement/location belongs to. Used for sorting pins in a grid if
    ///     applicable.
    /// </summary>
    internal static readonly MetadataProperty<AbstractPlacement, string> ModSource = new("ModSource",
        placement => { return $"{char.MaxValue}"; });
    ...
    internal static readonly MetadataProperty<AbstractPlacement, bool> DoNotMakePin =
        new("DoNotMakePin", placement => { return false; });
    ...
    internal static readonly MetadataProperty<AbstractPlacement, string> LocationPinSpriteKey =
        new("PinSpriteKey", placement => { return null; });
    ...
    internal static readonly MetadataProperty<AbstractPlacement, ISprite> LocationPinSprite =
        new("PinSprite", GetDefaultLocationSprite);

    /// <summary>
    ///     The pixel dimensions (x, y) of the location pin sprite, not including transparent pixels that may be in the
    ///     texture.
    ...
    internal static readonly MetadataProperty<AbstractPlacement, (int x, int y)?> LocationPinSpriteSize =
        new("PinSpriteSize", placement => { return null; });
    ...
    internal static readonly MetadataProperty<AbstractItem, string> ItemPinSpriteKey =
        new("PinSpriteKey", item => { return null; });

    /// <summary>
    ///     The sprite shown when spoilers are on or the pin is previewed, and this item is currently displayed in the animated
    ///     cycle.
    ///     If not provided, defaults to one based on the PoolGroup if possible. Otherwise, a question mark.
    /// </summary>
    internal static readonly MetadataProperty<AbstractItem, ISprite> ItemPinSprite =
        new("PinSprite", GetDefaultItemSprite);
    ...
    internal static readonly MetadataProperty<AbstractItem, (int x, int y)?> ItemPinSpriteSize =
        new("PinSpriteSize", item => { return null; });

    /// <summary>
    ///     The scenes for a multi-scene placement. The corresponding rooms are highlighted when the pin is selected.
    ///     The pin will also appear in a grid on the quick map, if one of the HighlightScenes is part of the current map zone
    ///     (and the placement is not cleared).
    /// </summary>
    internal static readonly MetadataProperty<AbstractPlacement, string[]> HighlightScenes =
        new("HighlightScenes", placement => { return null; });
    ...
    internal static readonly MetadataProperty<AbstractPlacement, (string scene, float x, float y)[]> MapLocations =
        new("MapLocations", placement => { return GetDefaultMapLocations(placement.Name); });
    ...
    internal static readonly MetadataProperty<AbstractPlacement, (string scene, float x, float y)[]> WorldMapLocations =
        new("WorldMapLocations", placement => { return GetDefaultWorldMapLocations(placement); });
    ...
    internal static readonly MetadataProperty<AbstractPlacement, (float x, float y)?> AbsMapLocation =
        new("AbsMapLocation", placement => { return null; });

    /// <summary>
    ///     Value to sort pins in the miscellaneous grid on the map.
    /// </summary>
    internal static readonly MetadataProperty<AbstractPlacement, int> PinGridIndex = new("PinGridIndex",
        placement => { return int.MaxValue; });

    /// <summary>
    ///     Optional text that can be revealed to help players find the placement's location. name = hint, logic = requirements
    ///     for the hint to be relevant.
    ...
    internal static readonly MetadataProperty<AbstractPlacement, RawLogicDef[]> LocationHints =
        new("LocationHints", placement => { return GetDefaultLocationHints(placement.Name); });

    internal static (string, float, float)[] GetDefaultMapLocations(string name)
    {
        if (Finder.TryGetLocation(name, out var mld))
            return mld.MapLocations.Select(mapLocation => ((string, float, float)) mapLocation).ToArray();

        ArchipelagoMapMod.Instance.LogDebug($"No MapLocationDef found for placement {name}");

        if (ItemChanger.Finder.GetLocation(name) is AbstractLocation al && al.sceneName is not null)
            return new (string, float, float)[] {new MapLocation {MappedScene = Finder.GetMappedScene(al.sceneName)}};

        ArchipelagoMapMod.Instance.LogDebug($"No MapLocation found for {name}.");
    ...
    private static (string, float, float)[] GetDefaultWorldMapLocations(AbstractPlacement placement)
    {
        if (placement is IPrimaryLocationPlacement locationPlacement
            ...
            && Finder.IsMappedScene(sceneName))
