...
using static ArchipelagoMapMod.RC.StateVariables.IHPStateManager;
...
public class HPStateManager : LogicVariable, IHPStateManager
{
    ...
    public override IEnumerable<Term> GetTerms()
    {
        ...
        {
            foreach (Term t in variable.GetTerms())
            {
                yield return t;
            }
        }
    ...
    private ISoulStateManager _ssm = null;
    protected ISoulStateManager SSM { get => _ssm ??= (ISoulStateManager)lm.GetVariableStrict(name: SoulStateManager.Prefix); set => _ssm = value; }
    ...
    {
        if (!pm.Has(Focus) || !IsHPDetermined(state))
        {
            return false;
        }

        foreach (EquipCharmVariable ecv in FocusCharms)
        {
            if (!ecv.IsDetermined(state))
            {
                return false;
            }
        }

        if (!SSM.TrySpendSoul(pm, ref state, 33))
        {
            return false;
        }
        ...
        if (state.GetInt(SpentHP) > 0)
        {
            state.SetInt(SpentHP, Math.Max(0, state.GetInt(SpentHP) - healAmt));
        }
    ...
    public StrictHPInfo GetHPInfo(ProgressionManager pm, LazyStateBuilder state)
    {
        ...
        if (FragileHeart.IsEquipped(state))
        {
            maxHp += 2;
        }

        if (JonisBlessing.IsEquipped(state))
        {
            maxHp = (int)(1.4f * maxHp);
        }

        int hp = maxHp - state.GetInt(SpentHP);
        int blueHP = -state.GetInt(SpentBlueHP);
        if (LifebloodHeart.IsEquipped(state))
        {
            blueHP += 2;
        }

        if (LifebloodCore.IsEquipped(state))
        {
            blueHP += 4;
        }
    ...
    protected virtual void DoHit(ref LazyStateBuilder state, HitInfo hit)
    {
        if (!hit.Survives)
        {
            throw new InvalidOperationException("DoHit called on lethal hit.");
        }
    ...
    public IEnumerable<LazyStateBuilder> TakeDamageSequence(ProgressionManager pm, LazyStateBuilder state, params int[] amounts)
    {
        if (!IsHPDetermined(state))
        {
            return DetermineHP(pm, state).SelectMany(s => TakeDamageSequence(pm, s, amounts));
        }
