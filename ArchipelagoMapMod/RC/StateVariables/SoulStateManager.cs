...
public class SoulStateManager : LogicVariable, ISoulStateManager
{
    ...
    public bool TryRestoreSoul(ProgressionManager pm, ref LazyStateBuilder state, int amount)
    {
        ...
        if (soulGain > 0)
        {
            state.Increment(SpentSoul, -soulGain);
        }

        if (reserveGain > 0)
        {
            state.Increment(SpentReserveSoul, -reserveGain);
        }
    ...
    {
        if (appliesToPriorPath && state.GetInt(MaxRequiredSoul) > limiter)
        {
            return false;
        }
